...
describe 'backups' do
  ...
  let(:manual_backup_command) do
    '/var/vcap/packages/service-backup/bin/manual-backup -config /var/vcap/jobs/redis-backups/config/backup-config.yml'
  end
  ...
  shared_examples "service backups are enabled" do
  ...
  context "shared vm plan" do
    ...
    it_behaves_like 'service backups are enabled'
  end
